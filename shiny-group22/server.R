...
shinyServer(function(input, output, session) {
  ...
  output$model_description_text1 <- renderText({
    "Best Subset Selection explores models of increasing size and picks the one with lowest cross-validated MSE."
  })
  ...
  output$lasso_pred_vs_actual <- renderPlot({
    ...
    ggplot(data.frame(Actual = df_test$Popularity, Predicted = as.numeric(preds)),
      ...
      theme_minimal() +
      theme(plot.title = element_text(hjust = 0.5))
    output$model_description_text2 <- renderText({
      "Lasso performs regularization to select features and shrink coefficients."
    })
